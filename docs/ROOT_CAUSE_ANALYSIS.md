...
- Add an automated connectivity check so contributors can validate the remote points to a reachable repository. The script will
  infer the expected URL automatically inside GitHub Actions from `GITHUB_SERVER_URL` and `GITHUB_REPOSITORY`, or you can set
  `EXPECTED_GIT_REMOTE` locally to the current GitHub URL so stale remotes are detected explicitly:
  ```sh
  EXPECTED_GIT_REMOTE=<NEW_REPO_URL> npm run check:git-remote
  ```
  The check sets `GIT_TERMINAL_PROMPT=0` to avoid hanging on credential prompts when the remote requires authentication and uses a non-interactive `GIT_SSH_COMMAND` (batch mode, strict host key checking, short timeout) so SSH remotes fail fast instead of stalling on prompts. When `AUTO_FIX_GIT_REMOTE=1` is provided alongside `EXPECTED_GIT_REMOTE`, the script will add or update `origin` before validating configuration so automation cannot be blocked by a missing or stale remote. The validator now normalizes SSH and HTTPS remotes to the same `<host>/<owner>/<repo>` slug so it will not fail just because the protocol differs from the expected URL; enable `AUTO_FIX_GIT_REMOTE=1` if you also want the protocol rewritten. Connectivity checks are opt-in (set `VERIFY_GIT_CONNECTIVITY=1`) to avoid hanging when the network cannot reach GitHub.
  To scope the current configuration without failing pipelines, you can run the check in status mode so it reports findings even when the remote is missing:
  ```sh
