...
if ! git remote get-url "$remote_name" >/dev/null 2>&1; then
  ...
  elif [[ -n "$report_only" ]]; then
    echo "Remote '$remote_name' is not configured." >&2
    echo "Report-only mode: configure the remote to enable connectivity checks." >&2
    exit 0
  else
...
Report-only mode: the remote was not changed; run with AUTO_FIX_GIT_REMOTE=1 EXPECTED_GIT_REMOTE set to update it automatically.
EOF
      exit 0
    else
      cat <<EOF >&2
...
if [[ -n "${VERIFY_GIT_CONNECTIVITY:-}" ]]; then
  ...
  echo "Connectivity check skipped (set VERIFY_GIT_CONNECTIVITY=1 to validate reachability)."
fi
